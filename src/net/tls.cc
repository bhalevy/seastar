        ...
        try {
            ...
            if (res < 0) {
                ...
                default:
                    ...
                    return handle_output_error(res).then_wrapped([this, res, alert = std::move(alert)] (future<> f) {
                        std::exception_ptr ex;
                        // output error future is expected to fail
                        if (f.failed()) {
                            ex = f.get_exception();
                        } else {
                            on_internal_error_noexcept(tlslog, "expected output error after do_handshake error");
                            ex = std::make_exception_ptr(std::system_error(res, glts_errorc));
                        }
                        return send_alert(GNUTLS_AL_FATAL, alert).then_wrapped([ex = std::move(ex)] (future<> f) mutable {
    ...
    ssize_t vec_push(const giovec_t * iov, int iovcnt) {
        ...
        try {
            ...
            if (_output_pending.failed()) {
                ...
                auto level = pending_error ? log_level::warn : log_level::debug;
