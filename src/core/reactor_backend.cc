...
void aio_storage_context::schedule_retry() {
    if (std::exchange(_retry_in_progress, true)) {
        return;
    }
    _pending_aio_retry_fut = _pending_aio_retry_fut.then([this] {
        // loop until both _aio_retries and _pending_aio_retry are empty.
        //
        // _aio_retries is exchanged with _pending_aio_retry
        // holding a batch of iocbs to retry.
        // In the meanwhile new retries may be queued onto _pending_aio_retry.
        ...
        return do_until([this] {
            ...
            }).then([this] (syscall_result<int> result) {
