...
public:
    ...
    void fail_after(uint64_t count, uint64_t how_long = std::numeric_limits<uint64_t>::max(), int error = 0) noexcept {
        ...
        _error = error ? error : _default_error;
        fsei_logger.debug("injector[{}] armed: error={} count={} fail_at={} fail_until={}", this, _error, _count, _fail_at, _fail_until);
    }
