...
auto recv_helper(signature<Ret (InArgs...)> sig, Func&& func, WantClientInfo wci, WantTimePoint wtp) {
    ...
    return [func = lref_to_cref(std::forward<Func>(func))](shared_ptr<server::connection> client,
        ...
        auto f = client->wait_for_resources(memory_consumed, timeout).then([client, timeout, msg_id, data = std::move(data), &func, &error_reply, h = std::move(h)] (auto permit) mutable {
            if (h->unregistered) {
...
lw_shared_ptr<rpc_handler> protocol<Serializer, MsgType>::get_handler(uint64_t msg_id) {
    ...
    return make_lw_shared<rpc_handler>();
}
