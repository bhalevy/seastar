...
// Make a shard-local alias of a remote buffer. No data is actually copied,
// only pointers.  The make_deleter callback takes ownership of the original
// buffer and is responsible for releasing it (typically via a batched_queue).
template <typename Buf, typename MakeDeleter>
Buf make_shard_local_buffer_copy(Buf* org, MakeDeleter make_deleter) {
    Buf buf(org->size);
