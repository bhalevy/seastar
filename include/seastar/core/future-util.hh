...
parallel_for_each(Range&& range, Func&& func) noexcept {
    auto impl = parallel_for_each_impl<Range, Func>;
    return futurize_apply(impl, std::forward<Range>(range), std::forward<Func>(func));
}
...
future<> do_for_each(Iterator begin, Iterator end, AsyncAction action) noexcept {
    ...
    return futurize_apply(func, std::forward<Iterator>(begin), std::forward<Iterator>(end), std::forward<AsyncAction>(action));
}
...
future<> do_for_each(Container& c, AsyncAction action) noexcept {
    ...
    return futurize_apply(func, c, std::forward<AsyncAction>(action));
}
...
do_when_all(FutureIterator begin, FutureIterator end) noexcept {
    ...
    return futurize_apply(func, std::forward<FutureIterator>(begin), std::forward<FutureIterator>(end));
}
