...
exception make_exception(std::derived_from<std::exception> auto&& e) noexcept {
    return exception(std::move(e));
}
...
[[nodiscard]]
exception return_exception(std::derived_from<std::exception> auto&& e) noexcept {
    return exception(std::move(e));
}
