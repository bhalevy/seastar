...
public:
    ...
    static future<> do_with_thread(config cfg, rpc::client_options co, thread_test_fn_with_client&& func) {
        return do_with(std::move(cfg), [func, co = std::move(co)] (rpc_test_env<MsgType>& env) {
            return seastar::async([&env, func, co = std::move(co)] {
                auto cl = env.make_rpc_client(co);
    ...
    static future<> do_with_thread(config cfg, thread_test_fn_with_client&& func) {
...
SEASTAR_TEST_CASE(test_rpc_connect) {
    ...
    for (auto i = 0; i < 2; i++) {
        for (auto j = 0; j < 4; j++) {
            ...
            auto f = rpc_test_env<>::do_with_thread(rpc_test_config({}, so), co, [] (rpc_test_env<>& env, test_rpc_proto::client& c1) {
...
SEASTAR_TEST_CASE(test_rpc_connect_multi_compression_algo) {
    ...
    return rpc_test_env<>::do_with_thread(rpc_test_config({}, so), co, [] (rpc_test_env<>& env, test_rpc_proto::client& c1) {
        env.register_handler(1, [](int a, int b) {
            return make_ready_future<int>(a+b);
        }).get();
        auto sum = env.proto().make_client<int (int, int)>(1);
...
SEASTAR_TEST_CASE(test_message_to_big) {
    return rpc_test_env<>::do_with_thread(rpc_test_config({0, 1, 100}, {}, true, false), [] (rpc_test_env<>& env, test_rpc_proto::client& c) {
        bool good = true;
...
SEASTAR_TEST_CASE(test_stream_simple) {
    ...
    return rpc_test_env<>::do_with(rpc_test_config({}, so), [] (rpc_test_env<>& env) {
...
SEASTAR_THREAD_TEST_CASE(test_rpc_scheduling_connection_based) {
    ...
    rpc_test_env<>::do_with_thread(rpc_test_config(limits), [sg1, sg2] (rpc_test_env<>& env) {
