...
SEASTAR_TEST_CASE(link_file_test) {
    return tmp_dir::do_with_thread([] (tmp_dir& t) {
        ...
        for (auto flag : all_flags) {
            remove_file(f2).get();
            BOOST_REQUIRE_NO_THROW(link_file_ext(f1, f2, flag).get());
        }
        ...
        BOOST_REQUIRE_EXCEPTION(link_file_ext(f1, f2, allow_same::no).get0(), std::system_error,
                testing::exception_predicate::message_contains("filesystem error: link failed: File exists"));
        // link_file_ext should succeed with allow_same::yes when f2 exists and same file
        BOOST_REQUIRE_NO_THROW(link_file_ext(f1, f2, allow_same::yes).get0());
        BOOST_REQUIRE(same_file(f1, f2).get0());
        ...
        BOOST_REQUIRE_EXCEPTION(link_file_ext(f1, f2, allow_same::no).get0(), std::system_error,
                testing::exception_predicate::message_contains("filesystem error: link failed: File exists"));
        BOOST_REQUIRE_EXCEPTION(link_file_ext(f1, f2, allow_same::yes).get0(), std::system_error,
                testing::exception_predicate::message_contains("filesystem error: link failed: File exists"));
        BOOST_REQUIRE(!same_file(f1, f2).get0());
