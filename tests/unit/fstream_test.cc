...
SEASTAR_TEST_CASE(test_file_input_stream_abort) {
    return tmp_dir::do_with_thread([] (tmp_dir& t) {
        ...
        for (size_t i = 0; i < file_size; ) {
            try {
                auto fut = in.read();
                if (successful_reads >= abort_at && !abort_requested) {
                    in.abort(std::make_exception_ptr(expected_exception()));
                    abort_requested = true;
                    BOOST_TEST_MESSAGE("Aborted input stream");
                }
                tmp = fut.get0();
                successful_reads++;
