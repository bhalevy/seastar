...
SEASTAR_THREAD_TEST_CASE(test_abort_when_either_already_aborted) {
    std::default_random_engine random_engine(testing::local_random_engine());
    std::uniform_int_distribution<> dist(0, 1);
    std::array<abort_source, 2> ass;
...
SEASTAR_THREAD_TEST_CASE(test_abort_when_either_move) {
    std::default_random_engine random_engine(testing::local_random_engine());
    std::uniform_int_distribution<> dist(0, 1);
    std::array<abort_source, 2> ass;
    auto awe0 = abort_when_either(ass[0], ass[1]);
