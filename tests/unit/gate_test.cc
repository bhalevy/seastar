...
static_assert(std::is_nothrow_default_constructible_v<gate_closed_exception>);
static_assert(std::is_nothrow_move_constructible_v<gate_closed_exception>);
static_assert(std::is_nothrow_move_assignable_v<gate_closed_exception>);
...
SEASTAR_TEST_CASE(named_gate_closed_test) {
    auto test_named_gate = [] (gate& g, const sstring name) -> future<> {
        if (!g.is_closed()) {
            BOOST_REQUIRE_NO_THROW(g.check());
            BOOST_REQUIRE_NO_THROW(g.enter());
            auto gh0 = g.try_hold();
    ...
    named_gate g2;
    co_await test_named_gate(g2, "named");
    BOOST_TEST_MESSAGE("test_named_gate(g2) after move");
    g2 = std::move(g1);
