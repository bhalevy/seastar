...
(the root is level -1 and contains no address), `index`
is the node's index in the sorted children list of that node,
and `pct` is the node's `total` weight relative to the parent
`total`, which is equal to the sum of all its children's
total stall time.
""")
