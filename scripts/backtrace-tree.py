...
def print_tree_backtraces(bt=[], n=root, level=-1, idx=0, rel=1.0):
    if level >= 0:
        avg = round(n.total/n.count) if n.count else 0
        s = f"[{level:>2}#{idx:<2} {round(100*rel):>3}%] {n.addr}"
        if resolver:
            s += ' ' + resolver.resolve_address(n.addr)
        bt = [s] + bt
    ...
    if children:
        i = 0
        for c in children:
            print_tree_backtraces(bt, c, level + 1, i, c.total / n.total if n.total else 1.0)
            i += 1
    else:
        sys.stdout.write(f"\ntotal={n.total} count={n.count} avg={avg}\n")
        for t in bt:
            sys.stdout.write(t)
